...
public class CreateBILFileFromExistingRasterUsingRBI {

  private static void RBI_CreateBILFileFromExistingRaster(String rasterFile,
      String outputFile) {
    ...
    try {
      ...
      outputDataset = rasterEngine.createDataSet(outputFile, "MI_BandInterleaved_BIL", rasterInfo,
      ...
      for (int fieldIndex = 0; fieldIndex < rasterInfo.getFieldInfos().size(); fieldIndex++) {
        for (int bandIndex = 0;
          ...
          RasterBandDataType type = rasterInfo.getFieldInfos().get(fieldIndex).getBandInfos()
              .get(bandIndex).getDataType();
  ...
  public static void main(String[] args) {
    try {
      String inputFilePath = filePath + "bil/10X10X200_Bit2_Convert.bil";
      String outputFilePath = filePath + "bil/OUTPUT_10X10X200_Bit2.bil";
      RBI_CreateBILFileFromExistingRaster(inputFilePath, outputFilePath);
      System.out.println("RBI Write Test for 10X10X200_Bit2_Convert is successful!");
    } catch (Exception ex) {
      throw ex;
    }
  }
